...
python servicenow_poll.py --change-number CHG0030001 \
  ...
  --task-description "Step-by-step remediation plan" \
...
Change tasks are only created when the change request is in **Scheduled** or **Implement** state. If the CR is Scheduled, it will be advanced to Implement before tasks are created. Otherwise, the script prints a message indicating that the CR is not in the right state.
